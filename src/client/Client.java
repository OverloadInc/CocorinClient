...
public class Client extends Thread {
     ...
     * @param nombre
     */
    Client(Chat chat, String host, Integer puerto, String nombre) {
        this.chat = chat;
        this.host=host;
        this.puerto=puerto;
    ...
    public void escuchar() {
        try {
            ...
            JOptionPane.showMessageDialog(null, "La comunicación con el servidor se ha\n"
        ...
        switch (tipo) {
            case "CONEXION_ACEPTADA":
                ...
                for(int i=2;i<lista.size();i++){
                    chat.addContacto(lista.get(i));
                }
                break;
            case "NUEVO_USUARIO_CONECTADO":
                // 1      - Identificador propio del cliente que se acaba de conectar
                chat.addContacto(lista.get(1));
                break;
            case "USUARIO_DESCONECTADO":
                // 1      - Identificador propio del cliente que se acaba de conectar
                chat.eliminarContacto(lista.get(1));
                break;
            case "MENSAJE":
                // 1      - Cliente emisor
                // 2      - Cliente receptor
                // 3      - Mensaje
                chat.addMensaje(lista.get(1), lista.get(3));
                break;
            default:
                break;
        }
